...
function formatTime(minutes, seconds) {
  ...
  let secondsText = sec;
  if (minutes < 10) {
    minutesText = mins.padStart(2, '0');
  }
  if (seconds < 10) {
    secondsText = sec.padStart(2, '0');
  }
  const result = `${minutesText}:${secondsText}`;
  return result;
}
...
function orderAlphabetically(str) {
  const indexes = [];
  for (let i = 0; i < str.length; i += 1) {
    indexes.push(str.codePointAt(i));
  }
  indexes.sort((a, b) => a - b);
  let result = '';
  for (let i = 0; i < indexes.length; i += 1) {
    result += String.fromCodePoint(indexes[i]);
  }
  return result;
}
...
function isPalindrome(str) {
  ...
  for (let i = str.length - 1; i >= 0; i -= 1) {
    if (
      ...
      reverse += str.charAt(i).toLowerCase();
    }
  }
  for (let i = 0; i < str.length; i += 1) {
    if (
  ...
  return reverse.toLowerCase() === string.toLowerCase();
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    let reverse = '';
    const word = words[i];
    for (let q = word.length - 1; q >= 0; q -= 1) {
      reverse += word[q];
    }
    if (i !== words.length - 1) {
      result += `${reverse} `;
...
function extractNameFromTemplate(value) {
  ...
  const second = words[2].split('!');
  return `${words[1]} ${second[0]}`;
}
...
function unbracketTag(str) {
  const tag = str.substring(1, str.length - 1);
  return tag;
}
...
function encodeToRot13(str) {
  const inputs = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  const outputs = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] !== ' ' && str[i] !== '!' && str[i] !== '?') {
      const char = str[i];
      const index = inputs.indexOf(char);
